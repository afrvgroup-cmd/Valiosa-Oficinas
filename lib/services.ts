...
export function createService(service: Omit<Service, "id" | "createdAt" | "status">): Service {
  if (typeof window === "undefined") throw new Error("Cannot create service on server")
...
export function getPerformanceStats() {
  ...
  const thisMonthServices = services.filter((s) => new Date(s.createdAt) >= thisMonth).length
